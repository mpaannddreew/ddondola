    ...
    <div class="card card-small mb-2 border">
        <div class="card-body p-4">
            <ul class="list-inline mb-0 lis-light-gold font-weight-normal h4 text-warning">
                ...
                <li class="list-inline-item"> {{ reviewable.average_rating }}</li>
            </ul>
            <small>{{ reviewable.average_rating }} average based on {{ reviewable.review_count }} {{ text }}</small>
        </div>
    </div>
</template>
...
<script>
    export default {
        ...
        computed: {
            text() {
                return this.reviewable.review_count === 1 ? 'rating': 'ratings';
            },
            starHtml() {
                var stars = "";
                var split_rating = this.reviewable.average_rating.split(".");
